This script is the Tidymodels workflow for an MLP model trained to predict rust tide occurrences on the Narragansett Bay Long Term Time Series. 
...
set.seed(15)
split <- initial_time_split(data, prop = 8.5/10, lag = 0)
train_raw <- training(split)
test_raw <- testing(split)

test <- test_raw |>
  dplyr::select( "count", "Surface Temp", "TotPAR", "flow_mean","Surface Salinity", "Wdir")|>
  tidyr::drop_na()
...
```{r}
mlnn <- mlp(mode = "classification", engine = "nnet", hidden_units = 2, epochs = 100, dropout = .1) 
...
rust_recipe <- train |> recipe(count ~ ., data = train)  |>
  ...
  step_upsample(count, over_ratio = .8) 
...
Making plots of model results and mlp structure
```{r}
set.seed(15)
data_conf <- count_env |>
  mutate(flownolag = flow_mean) |>
...
split <- initial_time_split(data_conf, prop = 8.5/10, lag = 0)
confcounts <- testing(split)


countcomp <- confcounts |>
  dplyr::select( "count", "Surface Temp", "TotPAR", "flow_mean", "Surface Salinity","Wdir", "Date") |>
  drop_na() 
